...
async function analyzeImage() {
    ...
    try {
        ...
        console.log("DEBUG: The given data is from backend ->", scores);
        console.log("DEBUG: The given tab is open ->", currentTab);
...
function loadProfile() {
    const p = JSON.parse(localStorage.getItem('exudetect_profile'));
    if (p) {
...
function saveProfile(e) {
    ...
    localStorage.setItem('exudetect_profile', JSON.stringify(profile));
    alert("Patient profile saved locally!");
}

function loadHistory() {
    const h = JSON.parse(localStorage.getItem('exudetect_history')) || [];
...
function saveHistory(mod, res, sev, conf) {
    ...
    localStorage.setItem('exudetect_history', JSON.stringify(h));
    loadHistory();
}

function clearHistory() {
    if (confirm('Are you sure you want to clear all locally saved scan history?')) {
        localStorage.removeItem('exudetect_history');
        loadHistory();
    }
}
...
function shareReport() {
    if (navigator.share) {
        navigator.share({
            title: 'ExuDetect Analysis Report',
...
window.onload = async () => {
    ...
    // Disclaimer logic
    if (!localStorage.getItem('exudetect_disclaimer_accepted')) {
...
function acceptDisclaimer() {
    localStorage.setItem('exudetect_disclaimer_accepted', 'true');
